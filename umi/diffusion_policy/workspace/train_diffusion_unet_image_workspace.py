...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if cfg.training.resume:
            ...
            # except:
            lastest_ckpt_path = cfg.training.ckpt_path
            accelerator.print(f"Resuming from checkpoint {lastest_ckpt_path}")
            self.load_checkpoint(path=lastest_ckpt_path)
                    ...
                    with torch.no_grad():
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                ...
                                scale = policy.normalizer['action'].params_dict['scale']
                                offset = policy.normalizer['action'].params_dict['offset']
                                gt_action = gt_action * scale  + offset
                                ...
                                print("Test Loss: " , step_log['train_action_mse_error'])
                                pred_loss += step_log['train_action_mse_error']
