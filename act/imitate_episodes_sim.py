...
def main(args):
    ...
    if is_eval:
        ...
        for ckpt_name in ckpt_names:
            if is_open_loop:
                eval_open_loop(config, ckpt_name, save_episode=True,val_dataloader=val_dataloader,train_dataloader=train_dataloader)
            else:
...
def eval_open_loop(config,ckpt_name,val_dataloader,train_dataloader,save_episode=True):
    set_seed(1000)
    ...
    with torch.inference_mode():
        policy.eval()
        epoch_dicts = []
        for batch_idx, data in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc="Evaluating"):
            forward_dict = forward_pass(data, policy)
            epoch_dicts.append(forward_dict)
        ...
        epoch_val_loss = epoch_summary['loss']
    print(f'Training loss:   {epoch_val_loss:.5f}')
    summary_string = ''
    for k, v in epoch_summary.items():
        summary_string += f'{k}: {v.item():.3f} '
    print(summary_string)
